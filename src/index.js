...
function fmt_array_uint8(arr, indices = []) {
...
class PBXCrc {
    ...
    updateByte(b) {
        const tbl_idx = (((this.sum >>> 0) ^ b) & 0xff);
        const tbl_val = PBXCrc.table[tbl_idx >>> 0];
        this.sum = tbl_val ^ (this.sum >>> 8) >>> 0;
    }

    updateBytes(bytes, start = 0, end) {
        bytes.forEach((b, idx) => {
    ...
    writeInvSum(buffer, offset = 0) {
        if (offset + PBXCrc.size > buffer.length) {
            throw new Error(
                "not enough room in buffer for crc: "
                + fmt_array_uint8(buffer, [offset, offset + PBCrc.size]))
        }
        const result = buffer.writeUInt32LE((this.sum ^ 0xffffffff) >>> 0, offset)
        return result;

    }

    toBytes() {
        const result = Buffer.alloc(PBXCrc.size, 0);
