import { fmt_array_uint8 } from "./formatters";

export const CRC_SIZE = 4;
export const CRC_TABLE = [0x00000000, 0x77073096,
...
export class PBXCrc {
    ...
    updateByte(b) {
        ...
        const tbl_val = CRC_TABLE[tbl_idx >>> 0];
    ...
    writeInvSum(buffer, offset = 0) {
        if (offset + CRC_SIZE > buffer.length) {
            throw new Error(
                ...
                + fmt_array_uint8(buffer, [offset, offset + CRC_SIZE]));
        }
    ...
    toBytes() {
        const result = Buffer.alloc(CRC_SIZE, 0);
